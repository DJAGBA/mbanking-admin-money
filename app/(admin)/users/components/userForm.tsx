...
import { UserData, CreateUserRequest } from '@/src/types/user';
...
export function UserForm({ user, onSubmit, onCancel }: UserFormProps) {
  ...
  // Local state initialized with fields.
  ...
  const handleSubmit = async (e: SyntheticEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // On attend l'exécution de la requête dans le parent
      await onSubmit(payload);
      
      // En cas de succès, le parent gère le message et on ferme tout de suite la modale
      onCancel();
        ...
        <form onSubmit={handleSubmit} className="flex flex-col max-h-[95vh]" autoComplete="off">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="flex flex-col gap-2 md:col-span-2">
                ...
                <div className="grid grid-cols-2 gap-3 p-4 border border-gray-200 rounded-lg bg-gray-50/50">
                  {['credit', 'debit', 'bank cashin', 'bank cashout'].map((op) => {
                ...
                {fieldErrors.operation && (
                  <p className="text-red-600 text-xs flex items-center gap-1"><AlertCircle size={12}/> {fieldErrors.operation}</p>
                )}
              </div>
          ...
          <div className="p-6 border-t bg-gray-50 flex gap-3 rounded-b-xl">
            <button 
              ...
              onClick={onCancel}
              disabled={loading}
              className="flex-1 px-4 py-2.5 bg-red-600 border border-red-600 rounded-lg font-semibold hover:bg-red-600 transition-all shadow-sm disabled:opacity-50 text-white"
            >
              Annuler
            </button>
            <button 
